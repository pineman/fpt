...
impl Breakpoint {
    fn check(&self, lr: &LR35902) -> bool {
        match self {
            ...
            Breakpoint::OnOpcode(opcode) => {
                lr.mem8(lr.pc()) == *opcode

            }
            Breakpoint::OnCB(opcode) => {
                lr.mem8(lr.pc()) == *opcode && lr.get_next_cb()
            }
        }
    }
}
...
impl Debugger {
    ...
    fn list_breakpoints(&self) -> String {
        self.breakpoints.iter().map(|breakpoint| breakpoint.to_string()).intersperse("\n".to_string()).collect::<String>()
    }
...
fn main() -> Result<()> {
    ...
    lua.set("continue", hlua::function0(move || -> LuaValue { 
        d1.borrow_mut().start();
        LuaValue::LuaNil
    }));

    let d1 = dbg_pointer.clone();
    lua.set("next", hlua::function0(move || -> LuaValue { 
        d1.borrow_mut().next();
        LuaValue::LuaNil
    }));

    let d1 = dbg_pointer.clone();
    lua.set("breakpoints", hlua::function0(move || -> LuaValue { 
        LuaValue::LuaString(d1.borrow_mut().list_breakpoints())
    }));

    let d1 = dbg_pointer.clone();
    lua.set("break", hlua::function1(move |opcode: u16| -> LuaValue { 
        d1.borrow_mut().set_breakpoint(Breakpoint::Breakpoint(opcode));
        LuaValue::LuaNil
    }));

    let d1 = dbg_pointer.clone();
    lua.set("on_opcode", hlua::function1(move |opcode: u8| -> LuaValue { 
        d1.borrow_mut().set_breakpoint(Breakpoint::OnOpcode(opcode));
        LuaValue::LuaNil
    }));

    let d1 = dbg_pointer.clone();
    lua.set("on_cb", hlua::function1(move |opcode: u8| -> LuaValue { 
        d1.borrow_mut().set_breakpoint(Breakpoint::OnCB(opcode));
        LuaValue::LuaNil
    }));

    let d1 = dbg_pointer.clone();
    lua.set("pc", hlua::function0(move || -> LuaValue { 
        LuaValue::LuaNumber(dbg!(d1.borrow_mut().pc().into()))
    }));

    lua.set("print", hlua::function1(move |s: String| -> LuaValue  { 
        println!("{}",s);
        LuaValue::LuaNil
    }));
